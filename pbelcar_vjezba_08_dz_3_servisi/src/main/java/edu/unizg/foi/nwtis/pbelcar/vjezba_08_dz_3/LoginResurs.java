...
public class LoginResurs {
  ...
  public Response postAuthAdmin(@HeaderParam("Accept") String tipOdgovora,
    ...
    if (k.size() == 1) {
      ...
      // Vjerojatno je ok da je hardkodirano za zadacu, ali bi trebalo iz konfig citati sve uloge i
      // ubaciti ih dinamicki
      if (g.contains("admin")) {
        this.kreirajNoviDnevnikrada(this.zahtjev.getRemoteAddr(), this.zahtjev.getLocalAddr(),
            korIme, serverProps.getProperty("os.name"), "Uspjesna autentikacija",
            serverProps.getProperty("java-version"));


        return Response.status(Response.Status.ACCEPTED).build();
      }
    }
    this.kreirajNoviDnevnikrada(this.zahtjev.getRemoteAddr(), this.zahtjev.getLocalAddr(), korIme,
        serverProps.getProperty("os.name"), "Neuspjesna autentikacija",
        serverProps.getProperty("java-version"));
